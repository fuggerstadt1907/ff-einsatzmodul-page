        ...
        return (
            <Stack direction='column' sx={{ m: 2 }} spacing={2}>
                < Stack direction={'row'} spacing={2}>
                    ...
                    <div onClick={() => onPressCategory('ABC')}><KategorieCard title="ABC ☢️" count={einsaetze?.filter(e => e.KAT === 'ABC').length ?? 0} /></div>
                    <div onClick={() => onPressCategory('Brand')}><KategorieCard title="Brand 🔥" count={einsaetze?.filter(e => e.KAT === 'Brand').length ?? 0} /></div>
                    <div onClick={() => onPressCategory('Fehlalarm')}><KategorieCard title="Fehlalarm ⚠️" count={einsaetze?.filter(e => e.KAT === 'Fehlalarm').length ?? 0} /></div>
                </Stack>
                < Stack direction={'row'} spacing={2}>
